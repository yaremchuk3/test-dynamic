...
class Ec2Inventory(object):
    ...
    def __init__(self):
        ...
        # Connection Type - ssm or ssh at the moment, used for ansible_host
        self.connection_type = os.getenv("CONNECTION_TYPE", "ssh")
    ...
    def read_settings(self):
        ...
        if 'auto' in self.regions:
            # DEEP-31438: Add support for multiple regions
            env_regions = os.environ.get('AWS_REGIONS')
            env_region = os.environ.get('AWS_REGION')
            default_region = os.environ.get('AWS_DEFAULT_REGION')
            if env_regions:
                self.regions = list({region.strip().lower() for region in env_regions.split(",")})
            elif env_region:
                self.regions = [str(env_region).strip().lower()]
            else:
                self.regions = [str(default_region).strip().lower()]
        ...
        self.iam_role = os.environ.get("IAM_ROLE")
        self.iam_external_id = os.environ.get("IAM_ROLE_EXTERNAL_ID")
    ...
    def add_instance(self, instance, region):
        ...
        if self.connection_type == "ssm":
            self.inventory["_meta"]["hostvars"][hostname]['ansible_host'] = instance.id
        else:
